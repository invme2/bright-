...
(function() {
    // Settings
    const buyURL = "https://spiritix.lemonsqueezy.com/buy/67f2d89b-8d0e-4c61-83cd-a0309d5e737f";

    const colors = [/*"#d0ff00",*/"#ef4444","#f97316","#f59e0b","#eab308","#84cc16","#22c55e","#10b981","#14b8a6","#06b6d4", "#5de3ff" /*"#0ea5e9"*/,"#3b82f6","#6366f1","#8b5cf6","#a855f7","#d946ef","#ec4899","#f43f5e"];
    const defaultColor = "#5de3ff";

    const grays = ["Slate","Gray","Zinc","Stone","Neutral",];
    const defaultGray = "Gray";
    const tones = ["Neutral", "Accent"];
    const defaultTone = "Neutral";

    const fonts = [
        {name:"System sans-serif", url: null, css: null, target:["body", "headings"]},
        {name:"Inter", url:"Inter", css:"'Inter'", target:["body", "headings"]},
        {name:"Roboto", url:"Roboto+Flex", css:"'Roboto Flex'", target:["body", "headings"]},
        {name:"Work Sans", url:"Work+Sans", css:"'Work Sans'", target:["body", "headings"]},
        {name:"Source Sans", url:"Source+Sans+3", css:"'Source Sans 3'", target:["body", "headings"]},
        {name:"Raleway", url:"Raleway", css:"'Raleway'", target:["body", "headings"]},
        {name:"Montserrat", url:"Montserrat", css:"'Montserrat'", target:["body", "headings"]},
        {name:"Nunito Sans", url:"Nunito+Sans", css:"'Nunito Sans'", target:["body", "headings"]},
        {name:"System serif", url: null, css: null, target:["body", "headings"], type: "serif"},
        {name:"Source Serif", url:"Source+Serif+4", css:"'Source Serif 4'", target:["body", "headings"], type: "serif"},
        {name:"Roboto Serif", url:"Roboto+Serif", css:"'Roboto Serif'", target:["body", "headings"], type: "serif"},
        {name:"Playfair Display", url:"Playfair+Display", css:"'Playfair Display'", target:["body", "headings"], type: "serif"},
    ];
    const defaultFontBody = "System sans-serif";
    const defaultFontHeadings = "System sans-serif";

    // Load font from Google Fonts
    const loadFont = (name, type="body") => {
        var selectedFont = fonts.find(f => f.name === name);
        if (!selectedFont || ["System sans-serif", "System serif"].includes(name)) return;

        var link = document.createElement('link');
        link.rel = "stylesheet";
        link.href = "https://fonts.googleapis.com/css2?family="+selectedFont.url+":ital,wght@0,400..900;1,400..900&display=swap";

        document.head.appendChild(link);

        if (type === "body") {
            document.documentElement.style.setProperty('--font-body', selectedFont.css);
        } else {
            document.documentElement.style.setProperty('--font-headings', selectedFont.css);
        }
    }     
    
    // Init Preact & Htm
    bindPreact.bind(window)();
    const { render, html, useState, useEffect } = window.htmPreact;
    ...
    function Popover(props) {
        ...
        const [selectedGray, setSelectedGray] = useState(defaultGray || grays[0]);
        const [selectedTone, setSelectedTone] = useState(defaultTone || tones[0]);
        const [selectedFontBody, setSelectedFontBody] = useState(defaultFontBody || fonts[0].name);
        const [selectedFontHeadings, setSelectedFontHeadings] = useState(defaultFontHeadings || fonts[0].name);

        const handleColorChange = (c) => {
            /** @global Spiritix */
            const [r,g,b] = window.Spiritix.hexToRgb(c);
            const [h,s,l] = window.Spiritix.rgbToHsl(r,g,b);
            const contrastColor = window.Spiritix.getContrastColor(r,g,b);

            document.documentElement.style.setProperty('--ghost-accent-color', c);
            document.documentElement.style.setProperty('--ghost-accent-color-rgb', `${r} ${g} ${b}`);
            document.documentElement.style.setProperty('--c-primary-hsl', `${h} ${s}% ${l}%`);
            document.documentElement.style.setProperty('--c-primary-hsl-50', `${h} 100% 90%`);
            document.documentElement.style.setProperty('--c-primary-hsl-950', `${h} 80% 10%`);
            document.documentElement.style.setProperty('--color-contrast', contrastColor);          
            setSelectedColor(c);
        };

        const handleGrayChange = (e) => {
            const gray = e.currentTarget.value;
            document.documentElement.classList.forEach(c => {
                if (c.startsWith("has-grayscale-")) {
                    document.documentElement.classList.remove(c);
                }
            });

            document.documentElement.classList.add("has-grayscale-" + gray);            
            setSelectedGray(gray);
        };

        const handleToneChange = (e) => {
            const tone = e.currentTarget.value;
            document.getElementById("sx-tone")?.classList.toggle('hidden', tone !== "Accent");
            setSelectedTone(tone);
        };

        const handleFontBodyChange = (e) => {
            const fontName = e.currentTarget.value;
            console.log("font body changed", fontName);
            setSelectedFontBody(fontName);

            document.documentElement.style.removeProperty('--font-body');
            document.documentElement.classList.remove('has-serif-body');

            if (fontName === "System sans-serif") return;
            ...
            setSelectedFontBody(fontName);                        
            loadFont(fontName, "body");
        }

        const handleFontHeadingsChange = (e) => {
            const fontName = e.currentTarget.value;
            console.log("font headings changed", fontName);
            setSelectedFontHeadings(fontName);   
            ...
            loadFont(fontName, "headings");
        }

        return html`
            <div id="sp-demo-popover" class="sp-demo-popover ${open && "open"}">
                <!-- Popover actions -->
                <div class="sp-demo-popover-actions">
                    <button id="sp-demo-popover-clear" class="sp-demo-popover-clear sx-hidden sx-bg-red-100 sx-py-0 sx-px-1.5 sx-rounded sx-cursor-pointer sx-text-left sx-ml-auto sx-mr-1 sx-text-[13px] sx-text-red-500 sx-min-h-6">
                        Clear
                    </button>
                    <button id="sp-demo-popover-close" class="sp-demo-popover-close sx-size-6 sx-flex sx-items-center sx-justify-center sx-rounded sx-bg-black/5 hover:sx-bg-black/10" title="Close" onClick=${() => setOpen(false)}>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" height="10" width="10"><path stroke-linecap="round" stroke-width="0.4" fill="currentColor" stroke="#000000" stroke-linejoin="round" d="M.44,21.44a1.49,1.49,0,0,0,0,2.12,1.5,1.5,0,0,0,2.12,0l9.26-9.26a.25.25,0,0,1,.36,0l9.26,9.26a1.5,1.5,0,0,0,2.12,0,1.49,1.49,0,0,0,0-2.12L14.3,12.18a.25.25,0,0,1,0-.36l9.26-9.26A1.5,1.5,0,0,0,21.44.44L12.18,9.7a.25.25,0,0,1-.36,0L2.56.44A1.5,1.5,0,0,0,.44,2.56L9.7,11.82a.25.25,0,0,1,0,.36Z"></path></svg>
                    </button>
                </div>
                ...
                <div class="sp-demo-popover-config">
                    ...
                    <div>
                        ${colors.map(c => html`
                            <button 
                                class="sp-demo-popover-color ${ c === selectedColor && "selected"}" 
                                style="background: ${c}"
                    ...
                    <div style="font-weight: normal; opacity: 0.8;">Current: <span class="sp-demo-popover-current">${selectedColor}</span></div>
                </div>
        
                <!-- Grayscale -->
                <label class="sp-demo-popover-config">
                    <div class="sp-demo-popover-heading">Grayscale</div>
                    <select id="sp-demo-popover-select-gray-scale" value="${selectedGray}" onChange=${handleGrayChange}>
                        ${grays.map(g => html`
                ...
                <!-- Tone -->
                <label class="sp-demo-popover-config">
                    <div class="sp-demo-popover-heading">Tone</div>
                    <select id="sp-demo-popover-select-tone" value="${selectedTone}" onChange=${handleToneChange}>
                        ${tones.map(t => html`
                ...
                <label class="sp-demo-popover-config">
                    ...
                    <select id="sp-demo-popover-select-text-font" value="${selectedFontBody}" onChange=${handleFontBodyChange}>
                        ${fonts.filter(f => f.type !== "serif" && f.target.includes('body')).map(f => html`
                ...
                <label class="sp-demo-popover-config">
                    ...
                    <select id="sp-demo-popover-select-headings-font" value="${selectedFontHeadings}" onChange=${handleFontHeadingsChange}>
                        ${fonts.filter(f => f.type !== "serif" && f.target.includes('headings')).map(f => html`
                            <option value="${f.name}">${f.name}</option>
                        `)}
                        <hr />
                        ${fonts.filter(f => f.type === "serif" && f.target.includes('headings')).map(f => html`
    ...
    function Widget (props) {
        ...
        return html`
            ...
            <div id="sp-demo-widget" class="sp-demo-widget">
                ...
                <!-- Customize Btn -->
                <button id="sp-demo-customize" class="sp-demo-btn" onClick=${() => setOpen((o) => !o)}>
                ...
                <!-- Buy Btn -->
                <a id="sp-demo-buy" href="${buyURL}" target="_blank" class="sp-demo-btn sp-demo-btn-primary">
    ...
    window.addEventListener("DOMContentLoaded", () => {
        ...
        root.id = "sx-demo-widget";
        document.body.appendChild(root);

        render(html`<${Widget} />`, root);


        const stickyLink = document.querySelector('#sx-course-link-sticky,#sx-showcase-link-sticky');
        const widget = document.getElementById('sp-demo-widget');

        if (stickyLink && widget) {
            const widgetRect = widget.getBoundingClientRect();

            const checkStickyLinks = () => {
                const linkRect = stickyLink.getBoundingClientRect();
                const shouldHide = (widgetRect.bottom <= linkRect.bottom && widgetRect.bottom > linkRect.top) || (widgetRect.top <= linkRect.bottom && widgetRect.top > linkRect.top)

                widget.classList.toggle('invisible', shouldHide);
            };
            
            window.addEventListener('scroll', () => {
                requestAnimationFrame(checkStickyLinks);
            }, { passive: true });
    ...
    // https://unpkg.com/browse/htm@3.1.1/preact/standalone.umd.js
    function bindPreact() {
        // @ts-ignore
